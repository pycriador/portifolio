/* ============================================
   APP.JS — Global Init, Rendering & Utilities
   ============================================ */
...
const App = (() => {
  const archIcons = [
    ...
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48l2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48l2.83-2.83"/></svg>',
  ];
    ...
    document.querySelectorAll('.copy-email').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(email).then(() => {
          const copiedText = typeof I18n !== 'undefined' ? I18n.get('contact.copied') : 'Copied!';
          const copyText = typeof I18n !== 'undefined' ? I18n.get('contact.copyEmail') : 'Copy Email';
          const toastMsg = typeof I18n !== 'undefined' ? I18n.get('toast.emailCopied') : 'Email copied';
          ...
          const failMsg = typeof I18n !== 'undefined' ? I18n.get('toast.emailFailed') : 'Failed to copy';
          showToast(failMsg);
        });
      });
    });
  ...
  function renderDynamicContent() {
    if (isHomePage() && typeof Projects !== 'undefined') {
      Projects.renderAll();
    }
    renderSkills();
    renderArchitecture();
  ...
  function renderSkills() {
    ...
    const categories = I18n.getArray('skills.categories');

    grid.innerHTML = categories.map((cat, i) => `
      <div class="skill-category" data-reveal data-delay="${Math.min(i + 1, 6)}">
        <h3 class="skill-category__title">
          <span class="skill-category__icon">${skillIcons[cat.name] || skillIcons.Frontend}</span>
          ${cat.name}
        </h3>
  ...
  function renderArchitecture() {
    ...
    const items = I18n.getArray('architecture.items');

    grid.innerHTML = items.map((item, i) => `
      <div class="arch-card" data-reveal data-delay="${Math.min(i + 1, 10)}">
        <div class="arch-card__icon">
          ${archIcons[i] || archIcons[0]}
        </div>
        <div>
          <h3 class="arch-card__title">${item.title}</h3>
          <p class="arch-card__description">${item.description}</p>
        </div>
      </div>
    `).join('');
  ...
  return { init, renderDynamicContent };
})();

document.addEventListener('DOMContentLoaded', () => {
  Navigation.init();
  if (typeof Projects !== 'undefined' && !window.location.pathname.includes('/projects/')) {
    Projects.init();
  }
  if (typeof I18n !== 'undefined') {
    I18n.init().then(() => App.init());
  } else {
    App.init();
  }
});
